...
export default function () {
  group('basic API health', () => {
    ...
    check(health, {
      ...
      'health response contains ok': (response) => response.body.includes('"status":"ok"')
    });
    ...
    check(version, {
      ...
      'version contains service': (response) => response.body.includes('service')
    });
  });
  ...
  group('generate smoke log', () => {
    ...
    const logResponse = http.post(`${BASE_URL}/api/logs/generate`, payload, jsonHeaders);
